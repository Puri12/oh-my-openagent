...
bunDescribe("sendSyncPrompt", () => {
  ...
  bunTest("applies agent tool restrictions for explore agent", async () => {
    ...
    //#then
    bunExpect(promptAsync).toHaveBeenCalled()
    bunExpect(promptArgs.body.tools["callomoagent"]).toBe(false)
  })
  ...
  bunTest("does not restrict callomoagent for sisyphus agent", async () => {
    //#given
    const { sendSyncPrompt } = require("./sync-prompt-sender")
    ...
    //#then
    bunExpect(promptAsync).toHaveBeenCalled()
    bunExpect(promptArgs.body.tools["callomoagent"]).toBe(true)
  })
