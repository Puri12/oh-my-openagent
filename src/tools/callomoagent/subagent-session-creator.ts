...
export async function resolveOrCreateSessionId(
	...
	if (args.session_id) {
		...
		if (sessionResult.error) {
			log("[callomoagent] Session get error", { error: sessionResult.error })
			return {
				ok: false,
	...
	const parentSession = await ctx.client.session
		...
		.catch((err: unknown) => {
			log("[callomoagent] Failed to get parent session", { error: String(err) })
			return null
		})
	...
	if (createResult.error) {
		log("[callomoagent] Session create error", { error: createResult.error })
